    ...
    ipcRenderer.invoke('select-Directory').then(res=>{
    ...
    ipcRenderer.invoke('dialog-show','请选择一个【pdf文档】')
    return
}
    if(!params.getData().output_dir){
    ipcRenderer.invoke('dialog-show','请选择一个【输出目录】保存分割后的图片')
    return
}
    ...
    await ipcRenderer.invoke('success')
